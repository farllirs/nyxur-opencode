...
export function showHelp() {
  ...
  const commands = [
    ...
    { cmd: '/update', desc: 'Actualizar el motor OpenCode' },
...
export function showDoctor(cwd, fix = false) {
  ...
  if (!binCheck.ok) fixes.push(() => downloadBinary(BIN_DIR));
...
export async function cmdUpdate() {
  console.log(`${colors.cyan}${colors.bold}╔══════════════════════════════════════╗${colors.reset}`);
  console.log(`${colors.cyan}${colors.bold}║     ACTUALIZAR MOTOR                ║${colors.reset}`);
  console.log(`${colors.cyan}${colors.bold}╚══════════════════════════════════════╝${colors.reset}\n`);

  if (existsSync(binaryPath)) {
    const size = (checkBinary(binaryPath).size / 1024 / 1024).toFixed(1);
    const ans = await question(` ${colors.green}?${colors.reset} Binario actual (${size} MB). ¿Reemplazar? [y/N] `);
    if (ans.toLowerCase() !== 'y' && ans.toLowerCase() !== 'yes') {
      console.log(` ${colors.yellow}⚠ Cancelado${colors.reset}\n`);
      return;
    }
    unlinkSync(binaryPath);
    console.log(`${colors.green}✔${colors.reset} Binario antiguo eliminado\n`);
  }

  if (downloadBinary(BIN_DIR)) {
    console.log(`\n${colors.green}${colors.bold}✔ Motor actualizado a v${ENGINE_VERSION}${colors.reset}\n`);
  } else {
    console.log(`\n${colors.red}${colors.bold}✘ Error en la actualización${colors.reset}\n`);
  }
}
